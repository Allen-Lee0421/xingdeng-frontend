// 1. 【核心對齊】鎖定目前的 Online 隧道網址
const API_BASE_URL = "https://mariyah-unexplanatory-regan.ngrok-free.dev"; 

// 2. 左框：命理推演功能
async function startAnalysis() {
    const status = document.getElementById('statusOutput') || document.querySelector('.status-message');
    ...
    try {
        ...
        if (response.ok) {
            ...
            status.innerText = "連線成功但後端未回傳數據 (502)，請確認 Docker 容器已啟動。"; //
...
function startFraudScan() {
    const input = prompt("請貼上要偵測的網址或訊息：");
    if(input) {
